Notes
=====
Tested on OSX only with clang compiler. Must build boost with the same compiler as used for QLib. If necessary run aclocal and automake commands to regenereate make.am files from make.in files:
sh autogen.sh
aclocal
automake
./configure --disable-dependency-tracking
./configure_clang_osx needed to prevent name mangling issue in build of master [or use the appropriate OS and compiler version]

Further documentation on CVA methodology in docs folder
