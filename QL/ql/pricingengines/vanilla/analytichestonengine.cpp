...
namespace QuantLib {
    ...
    Real AnalyticHestonEngine::Fj_Helper::operator()(Real phi) const
    {
      ...
      //MFD const std::complex<Real> addOnTerm
      // MFD     = engine_ > 0 ? engine_->addOnTerm(phi, term_, j_) : Real(0.0);
        const std::complex<Real> addOnTerm = engine_->addOnTerm(phi, term_, j_); 
        if (cpxLog_ == Gatheral) {
            if (phi != 0.0) {
                if (sigma_ > 1e-5) {
